...
import { refrigeratorData } from '../assets/refrigerator/refrigeratorData.js.js';
export default function Refrigerator() {
   const [currentPage, setCurrentPage] = useState(1);
    const [itemPerPage, setItemPerPage] = useState(8);
    const [filterRefrigerator, setFilterRefrigerator] = useState(refrigeratorData);
    const dispatch = useDispatch()
    const lastIndex = currentPage * itemPerPage;
    const firstIndex = lastIndex - itemPerPage;

    const sliceRefrigerator = filterRefrigerator.slice(firstIndex, lastIndex);
    const pages = [];

    for (let i = 1; i < filterRefrigerator.length / itemPerPage; i++) {
      pages.push(i);
    }


    const uniqueRefrigerator = [
      "All",
      ...new Set(refrigeratorData.map((item) => item.category)),
    ];

    const handlePode = (category) => {
      if (category === "All") {
        setFilterRefrigerator(refrigeratorData);
      } else {
        const filterProducts = refrigeratorData.filter(
          (item) => item.category === category
        );
        setFilterRefrigerator(filterProducts);
      }
    };
  return (
