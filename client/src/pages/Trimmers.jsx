...
import { trimmersData } from '../assets/trimmers/trimmerData.js'

export default function Trimmers() {
  const [currentPage, setCurrentPage]= useState(1)
      const [itemPerPage, setItemPerPage] = useState(8)
      const [filterTrimmer, setFilterTrimmer] = useState(trimmersData)
  
      const uniqueTrimmer = ["All",...new Set(trimmersData.map((item)=>item.category))]
         const handleTrimer =(category)=>{
          if(category==="All"){
            setFilterTrimmer(trimmersData)
          }else{
            const filterProducts = trimmersData.filter((item)=>item.category===category)
                 setFilterTrimmer(filterProducts)
          }
         }
      
          const lastIndex = currentPage * itemPerPage;
          const firstIndex = lastIndex - itemPerPage;
          const sliceTrimmer = filterTrimmer.slice(firstIndex, lastIndex);
          const pages = [];
          for (let i = 1; i < filterTrimmer.length / itemPerPage; i++) {
            pages.push(i);
          }
  return (
                   ...
                   <div className="w-full h-auto flex flex-col gap-5 md:flex-row mt-10 ">
                     <div className="flex flex-col gap-3">
                       ...
                       <div className="w-full md:w-60 gap-4 flex flex-wrap md:gap-1 flex-row md:flex-col px-2">
                         {uniqueTrimmer.map((item) => (
                           <span key={item}
                             onClick={() => handleTrimer(item)}
