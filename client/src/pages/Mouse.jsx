...
import { mouseData } from "../assets/mouse/mouseData.js";

export default function Mouse() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemPerPage, setItemPerPage] = useState(8);
  const [filterMouse, setFilterMouse] = useState(mouseData);

  const dispatch = useDispatch();


  const uniqueMouse = ["All",...new Set(mouseData.map((item)=>item.category))]
  const handleMouse=(category)=>{
      if(category==="All"){
        setFilterMouse(mouseData)
      }else{
       const filterProducts = mouseData.filter((item)=>item.category===category)
            setFilterMouse(filterProducts)
      }
  }
  const lastIndex = currentPage * itemPerPage;
  const firstIndex = lastIndex - itemPerPage;

  const sliceMouse = filterMouse.slice(firstIndex, lastIndex);
  const pages = [];

  for (let i = 1; i < filterMouse.length / itemPerPage; i++) {
    pages.push(i);
  }
