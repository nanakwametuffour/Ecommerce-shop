...
import { tvData } from '../assets/TV/tvData.js';
...
export default function Tv() {
   const [currentPage, setCurrentPage] = useState(1);
    ...
    const [filterTv, setFilterTv] = useState(tvData)
    const dispatch= useDispatch()


    const uniqueTv = [
        "All",
        ...new Set(tvData.map((item) => item.category)),
      ];
    
      const handleTv = (category) => {
       
        if (category === "All") {
          setFilterTv(tvData);
        } else {
          const filterItem = tvData.filter(
            (item) => item.category === category
          );
          setFilterTv(filterItem);
        }
      };
      ...
      const sliceProducts = filterTv.slice(
        firstIndex,
        lastIndex
      );
      const pages = [];

      for (let i = 1; i < filterTv.length / itemPerPage; i++) {
        pages.push(i);
      }
  return (
