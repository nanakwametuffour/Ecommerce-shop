...
import { speakersData } from '../assets/speakers/speakerData.js'
...
export default function Speaker() {
  ...
  const [filterSpeaker, setFilterSpeaker] = useState(speakersData)
   const dispatch = useDispatch()
  const uniqueSpeaker = ["All",...new Set(speakersData.map((item)=>item.category))]
   const handleSpeaker =(category)=>{
    if(category==="All"){
      setFilterSpeaker(speakersData)
    }else{
      const filterProducts = speakersData.filter((item)=>item.category===category)
           setFilterSpeaker(filterProducts)
    }
   }
   ...
   const sliceSpeaker = filterSpeaker.slice(firstIndex, lastIndex)
    const pages = []
    for(let i = 1; i < filterSpeaker.length / itemPerPage; i++){
       pages.push(i)
    }
  return (
           ...
           <div className="w-full h-auto flex flex-col gap-5 md:flex-row mt-10 ">
             <div className="flex flex-col gap-3">
               ...
               <div className="w-full md:w-60 gap-4 flex flex-wrap md:gap-1 flex-row md:flex-col px-2">
                 {uniqueSpeaker.map((item) => (
                   <span key={item}
                     onClick={() => handleSpeaker(item)}
