...
import { camerasData } from '../assets/camera/cameraData.js';
export default function Camera() {
    const [currentPage, setCurrentPage]= useState(1)
    const [itemPerPage, setItemPerPage] = useState(8)
    const [filterCamera, setFilterCamera] = useState(camerasData)

    const uniqueCamera = ["All",...new Set(camerasData.map((item)=>item.category))]
       const handleCamera =(category)=>{
        if(category==="All"){
          setFilterCamera(camerasData)
        }else{
          const filterProducts = camerasData.filter((item)=>item.category===category)
               setFilterCamera(filterProducts)
        }
       }
    
        const lastIndex = currentPage * itemPerPage;
        const firstIndex = lastIndex - itemPerPage;
        const sliceCamera = filterCamera.slice(firstIndex, lastIndex);
        const pages = [];
        for (let i = 1; i < filterCamera.length / itemPerPage; i++) {
          pages.push(i);
        }
  return (
               ...
               <div className="w-full h-auto flex flex-col gap-5 md:flex-row mt-10 ">
                 <div className="flex flex-col gap-3">
                   ...
                   <div className="w-full md:w-60 gap-4 flex flex-wrap md:gap-1 flex-row md:flex-col px-2">
                     {uniqueCamera.map((item) => (
                       <span key={item}
                         onClick={() => handleCamera(item)}
